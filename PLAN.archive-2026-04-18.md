...
### OpenClaw 機能の Airi 内再実装 (OC-Features)

> 参照実装: [openclaw/openclaw](https://github.com/openclaw/openclaw) — MIT。Gateway daemon / 20+ channel / pairing / sandbox は移植対象外。自律エージェント + スキル + Cron の 3 機能を Airi の injeca + Eventa + Pinia で再実装する。
>
> **開始日**: 2026-04-16
> **対象**: `apps/stage-tamagotchi` (Electron) 優先、platform-agnostic core は `packages/` に切り出し
> **非破壊制約**: 既存 Chat / Provider / Claude Code Integration / TTS / VRM は一切変更しない。Agent 機能は feature flag (`settings.agentRuntime.enabled`, default: `false`) で opt-in。

| Phase | 概要 | 状態 | 完了日 | 備考 |
|---|---|---|---|---|
| OC-AG-1 | Agent Harness コア (domain logic) | ✅ 完了 | 2026-04-16 | run-attempt loop / plan / tool-call / abort |
| OC-AG-2 | Skill Registry (SKILL.md ローダー) | ✅ 完了 | 2026-04-17 | workspace discovery + manifest parse |
| OC-AG-3 | Cron Scheduler | ✅ 完了 | 2026-04-17 | cron parser + persistent store + trigger |
| OC-AG-4 | Electron 統合 (injeca + Eventa) | ✅ 完了 | 2026-04-17 | main-side service + IPC contracts |
| OC-AG-5 | Renderer Pinia Store + 最小 UI | ✅ 完了 | 2026-04-17 | Pinia store + settings page + devtools page |
| OC-AG-6 | Model Driver bridge (既存 Provider 再利用) | ✅ 完了 | 2026-04-18 | Option A 採用。renderer harness + xsai driver + cron broadcast |
| OC-AG-7 | 承認フロー UI (approval) | ✅ 完了 | 2026-04-18 | interactive gate + pending store + modal + composable |
| OC-AG-8 | テスト + i18n + ドキュメント | ✅ 完了 | 2026-04-18 | en/ja/zh-Hans + README + 3 integration docs + 統合テスト (gated) |
...
## OC-Features: OpenClaw 機能の Airi 内再実装

> 参照元 (MIT): OpenClaw Plugin SDK (`plugins/sdk-agent-harness`, `plugins/sdk-overview`, `cli/skills`, `tools/cron`)。実装は参照のみ行い、コードは**取り込まない**。Airi 流 (injeca + Eventa + Pinia + Vue + Valibot) で再設計する。

### 全体原則 (全 OC-AG-* 共通)

1. **非破壊**: 既存ファイルへの変更は最小限。既存 store / provider / component への追加はすべて optional + default 値で、未設定なら現状と同じ振る舞い。
2. **Feature flag**: `settings.agentRuntime.enabled` が `false` のとき Agent runtime は初期化されない (injeca builder が no-op を返す)。デフォルト `false`。
3. **Platform-agnostic core は `packages/`**: Electron 非依存のコアは `packages/agent-runtime` / `packages/skill-registry` / `packages/cron-runtime` に置く。Electron 固有 wiring は `apps/stage-tamagotchi/src/main/services/airi/agent/` に集約。
4. **既存基盤の流用**: MCP subprocess 基盤 (`apps/stage-tamagotchi/src/main/services/airi/mcp-servers/`)、Claude Code Integration (`services/airi/claude-code/`)、Provider registry (`packages/stage-ui/src/libs/providers`)、Stream store (`packages/stage-ui/src/stores/stream-store.ts`) を再利用する。
5. **Valibot でスキーマ検証**: manifest / config / cron は Valibot schema を用意 (AGENTS.md の方針)。
6. **Eventa contract を先に固める**: 全 Phase で新規 IPC は `apps/stage-tamagotchi/src/shared/eventa.ts` に集約。
7. **TDD**: 各 Phase で赤テスト先行、ドメインロジックは Electron 非依存で単体テスト可能に保つ。
8. **`errorMessageFrom(@moeru/std)` を使う**: エラー整形は `error instanceof Error ? ... : String(error)` パターンを避ける。
...
┌─────────────── Airi Electron ────────────────────────────┐
│  Renderer (Vue)                                          │
│   settings/agent-runtime.vue  ─ toggle / skill list / cron │
│   devtools/agent-runs.vue     ─ live run log              │
│   stores/agent-runtime.ts     ─ Pinia (OC-AG-5)          │
│         │ Eventa                                          │
│  Main   ▼                                                 │
│   ┌────────────────────────────────────────────────────┐ │
│   │ createAgentService (injeca, OC-AG-4)               │ │
│   │  ├─ AgentHarness   (OC-AG-1)                       │ │
│   │  │   ├─ ModelDriver (OC-AG-6) → xsai / claude-code │ │
│   │  │   ├─ ToolInvoker (MCP servers を流用)            │ │
│   │  │   └─ ApprovalGate (OC-AG-7)                     │ │
│   │  ├─ SkillRegistry  (OC-AG-2)                       │ │
│   │  │   └─ SKILL.md loader (workspace tail)           │ │
│   │  └─ CronScheduler  (OC-AG-3)                       │ │
│   │      └─ JobStore (sqlite or JSON)                  │ │
│   └────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────┘
              ▲ 既存 MCP / Claude Code は並列維持・無変更
```
...
| パッケージ | 役割 | 依存 |
|---|---|---|
| `packages/agent-runtime` | Harness, run-attempt loop, plan, abort, event bus | `@moeru/std`, `valibot`, `@moeru/eventa` (types only) |
| `packages/skill-registry` | `SKILL.md` loader, matcher, registry Map | `@moeru/std`, `valibot`, `gray-matter` (frontmatter) |
| `packages/cron-runtime` | cron パーサ + スケジューラ + JobStore interface | `@moeru/std`, `valibot`, `cron-parser` |

すべて `tsdown` で bundle (`packages/vite-plugin-warpdrive` のパターン)。

---

## Phase OC-AG-1: Agent Harness コア (domain logic)

**目的**: モデル / チャネル / Electron に一切依存しない `AgentHarness` を作り、run-attempt loop + tool-call loop + abort / reset を pure TypeScript で確立する。

**配置**: `packages/agent-runtime/`

### ファイル構成
- [x] `packages/agent-runtime/package.json` (`@proj-airi/agent-runtime`)
- [x] `packages/agent-runtime/tsdown.config.ts` — `packages/vite-plugin-warpdrive` を参考
- [x] `packages/agent-runtime/src/index.ts` — 公開 API の re-export
- [x] `packages/agent-runtime/src/types.ts` — `AgentTurn` / `AgentPrompt` / `AgentEvent` / `PartialReply` / `StopReason` / `ToolDefinition`
- [x] `packages/agent-runtime/src/harness.ts` — `createAgentHarness({ modelDriver, toolInvoker, approvalGate }): AgentHarness`
- [x] `packages/agent-runtime/src/run-attempt.ts` — `runAttempt(params: RunAttemptParams): Promise<AttemptResult>`
- [x] `packages/agent-runtime/src/tool-loop.ts` — tool_call → approval → invoke → tool_result → re-invoke model のループ
- [x] `packages/agent-runtime/src/abort.ts` — `AbortSignal` 連動、session reset、mid-stream cancel
- [x] `packages/agent-runtime/src/event-bus.ts` — `createAgentEventBus()` (mitt ライクな minimal pub/sub)
- [x] `packages/agent-runtime/src/schemas.ts` — Valibot schema (RunAttemptParams / AgentEvent)
- [x] `packages/agent-runtime/README.md` — 「何をするか / どう使うか / いつ使わないか」

### 主要インターフェース (参考実装 from OpenClaw)
```ts
...
### テスト (TDD 先行)
- [x] `run-attempt.test.ts` — 最小 1 ターン (plan → text-delta → finish) / mid-stream abort / model error 伝搬 (7 ケース)
- [x] `tool-loop.test.ts` — single tool-call → approved → result → re-invoke / rejected / approval timeout / max-iterations guard (複数ケース)
- [x] `abort.test.ts` — tool invoke 中 abort → ToolInvoker.cancel が呼ばれる / reset で session state が消える
- [x] `event-bus.test.ts` — subscribe/unsubscribe, listener 例外 isolation
- [x] `schemas.test.ts` — Valibot schema の正常系/境界
- [x] `sensitivity.test.ts` — allow-list 評価 (10 ケース)

### 品質ゲート
- [x] `pnpm -F @proj-airi/agent-runtime typecheck` pass
- [x] `pnpm -F @proj-airi/agent-runtime exec vitest run` all pass (55 / 55), coverage ≥ 80% (90.57%)
- [x] `eslint` 0 errors (touched files)
- [x] **非破壊**: 既存パッケージのテストがすべて通る (`pnpm test:run`) — 新規パッケージ追加のみ、既存コード変更なし
- [ ] `pnpm -F @proj-airi/agent-runtime build` (tsdown) 成功、dts 生成 — OC-AG-4 で Electron 統合時に実施

### 実績ログ

#### 2026-04-16 — OC-AG-1 完了
- `packages/agent-runtime/` 新規追加 (`@proj-airi/agent-runtime`): platform-agnostic な pure TypeScript core。Electron / Vue / Pinia 非依存。
- **公開 API**: `createAgentHarness` / `runAttempt` / `handleToolCall` / `evaluateSensitivity` / `createAgentEventBus` / `linkAbortSignals` / `createResetController` / `isAbortError` / `createAbortError` / `runAttemptParamsSchema` / `agentEventSchema`。
- **AgentMessage.toolCalls**: OpenAI / Anthropic tool-use protocol と整合させるため `toolCalls: ToolCallRequest[]` を構造化フィールドで保持 (fake string 形式ではない)。
- **Sensitivity 判定**: `CATEGORY_RULES` テーブルで prefix → category を一意にマッチ。`exec` が `executor` にマッチする over-match bug を修正。
- **AbortSignal.any fallback**: `combineAbortSignalsManually` で listener leak を明示 (NOTICE 付き)。per-turn 寿命なので許容。
- **max_tool_calls guard**: budget ベースでアシスタント record にも dispatched 分のみ含めるよう整合。
- **レビュー体制**: 並列 3 エージェント (code-reuse / code-quality / efficiency) で diff 全体をレビュー、指摘を反映済み。
- **テスト**: 55 / 55 pass、coverage 90.57%。
- **typecheck / lint**: clean。
- **非破壊確認**: 既存ワークスペースへの import / 既存ファイル変更は `vitest.config.ts` の projects 追加 1 行のみ。

**次のフェーズ**: OC-AG-2 (Skill Registry) と OC-AG-3 (Cron Scheduler) は並列実装可能。

---

## Phase OC-AG-2: Skill Registry (SKILL.md ローダー)

**目的**: OpenClaw の `skills/<skill>/SKILL.md` 形式を踏襲し、Airi workspace (`~/.airi/skills/` + `packages/stage-ui/skills/` built-in) からスキル定義を動的ロードする。

**配置**: `packages/skill-registry/`

### SKILL.md フォーマット (OpenClaw 準拠 + 簡素化)
```markdown
---
id: web-search
name: Web Search
...
- Prefer fetch for simple GET requests
- Use read only for allowed domains
```

### ファイル構成
- [x] `packages/skill-registry/package.json` (`@proj-airi/skill-registry`)
- [x] `packages/skill-registry/tsdown.config.ts`
- [x] `packages/skill-registry/src/types.ts` — `SkillDefinition` / `SkillManifest` / `SkillMatcher`
- [x] `packages/skill-registry/src/load-skill.ts` — `loadSkillFromDirectory(dir): Promise<SkillDefinition>` (frontmatter parse + body 抽出)
- [x] `packages/skill-registry/src/registry.ts` — `createSkillRegistry()`, `register` / `resolve(prompt) → SkillDefinition[]`
- [x] `packages/skill-registry/src/workspace-discovery.ts` — `discoverSkills(roots: string[]): Promise<SkillDefinition[]>` (builtin + user workspace)
- [x] `packages/skill-registry/src/schemas.ts` — Valibot schema for frontmatter
- [x] `packages/skill-registry/src/matcher.ts` — regex triggers (WeakMap 正規表現キャッシュ付き)
- [x] `packages/skill-registry/src/index.ts` — public API
- [x] `packages/skill-registry/README.md`

### 依存追加
- [x] `gray-matter` (frontmatter parser) — `^4.0.3` (direct pin, workspace catalog 未使用)
- [x] `micromatch` — 依存には含めたが file pattern trigger は OC-AG-8 で拡張 (現状は regex triggers のみ)

### テスト (TDD 先行)
- [x] `load-skill.test.ts` — 正常 SKILL.md / frontmatter 欠損 / invalid YAML / body 空 / required field 欠損
- [x] `matcher.test.ts` — trigger regex 複数ヒット優先度 / 大文字小文字 / 空 triggers (6 ケース)
- [x] `registry.test.ts` — register / duplicate id reject / resolve by prompt / clear (5 ケース)
- [x] `workspace-discovery.test.ts` — tmp dir fixture で union / 存在しない root skip / SKILL.md 欠損 warn / duplicate id first-wins (4 ケース)
- [x] `schemas.test.ts` — Valibot 正常/異常 (5 ケース)

### 品質ゲート
- [x] `pnpm -F @proj-airi/skill-registry typecheck` pass
- [x] `pnpm -F @proj-airi/skill-registry exec vitest run` all pass (30 / 30), coverage ≥ 80% (92.1%)
- [x] `eslint` 0 errors
- [x] **非破壊**: 既存パッケージは一切変更なし。`vitest.config.ts` の projects 追加 1 行のみ

### 実績ログ

#### 2026-04-17 — OC-AG-2 完了
- `packages/skill-registry/` 新規追加 (`@proj-airi/skill-registry`): Node `fs/promises` のみに依存する pure TS ローダー。
- **公開 API**: `createSkillRegistry` / `loadSkillFromDirectory` / `discoverSkills` / `matchPrompt` / `compileTriggers` / `skillManifestSchema`。
- **Regex cache**: `matcher.ts` で `WeakMap<SkillDefinition, RegExp[]>` による遅延コンパイル。triggers は immutable なので `resolve()` 毎の再コンパイルを回避。
- **Duplicate detection**: registry に `Set<string>` を並走させて O(1) で重複 id を検出。
- **TOCTOU 排除**: `workspace-discovery` の事前 `stat(SKILL.md)` プローブを削除し、`loadSkillFromDirectory` の ENOENT で warn に統一。
- **レビュー体制**: 並列 3 エージェント (code-reuse / code-quality / efficiency) の指摘を反映済み。
- **テスト**: 30 / 30 pass、coverage 92.1%。

### 申し送り事項
- `tools:` エントリは MCP server で既に登録済みのツール名と照合する (名前空間衝突時はエラー) — OC-AG-4 で MCP registry と結線予定。
- built-in skills は空で開始 (最初は user workspace のみ)。将来 `packages/skills-builtin/` を作って web-search / file-ops / memory 等を同梱。
- file pattern trigger は現状未実装。必要になったら `micromatch` を使って `matcher.ts` に追加する (依存は既に入れてある)。

---

## Phase OC-AG-3: Cron Scheduler

**目的**: Airi キャラクターが能動的に話しかけたり定期タスクを走らせたりする基盤として、cron 式でジョブを登録・実行するスケジューラを実装。

**配置**: `packages/cron-runtime/` + `apps/stage-tamagotchi/src/main/services/airi/cron/`

### ファイル構成 (packages/cron-runtime)
- [x] `packages/cron-runtime/package.json` (`@proj-airi/cron-runtime`)
- [x] `packages/cron-runtime/tsdown.config.ts`
- [x] `packages/cron-runtime/src/types.ts` — `CronJob` / `CronTriggerEvent` / `JobStore` interface
- [x] `packages/cron-runtime/src/scheduler.ts` — `createCronScheduler({ store, clock, onTrigger })` — 単一 `setTimeout` 再計画方式
- [x] `packages/cron-runtime/src/job-store.memory.ts` — in-memory JobStore (テスト用)
- [x] `packages/cron-runtime/src/job-store.json.ts` — JSON 永続化 (`~/.airi/cron/jobs.json` 想定 / 破損時は backup + empty)
- [x] `packages/cron-runtime/src/schemas.ts` — Valibot
- [x] `packages/cron-runtime/src/clock.ts` — `Clock` interface + `SystemClock` / `createFakeClock()`
- [x] `packages/cron-runtime/src/index.ts`
- [x] `packages/cron-runtime/README.md`

### 依存追加
- [x] `cron-parser@^5.5.0` (API: `CronExpressionParser.parse(expr, { currentDate, tz }).next().toDate()`)
...
export interface CronJob {
  ...
  prompt: string          // agent に投げるプロンプト
  sessionId?: string      // continuation
  enabled: boolean
  skillId?: string        // 特定スキルを強制起動
  timezone?: string
  lastRunAt?: string
  nextRunAt?: string
}
...
### テスト (TDD 先行)
- [x] `scheduler.test.ts` — FakeClock で trigger 時刻制御 / enabled=false skip / nearest-first / addJob while running / removeJob cancels / stop() 後 fire しない / toggleJob 再計画 / addJob の nextRunAt 算出
- [x] `job-store.json.test.ts` — 欠損 → []/ 往復 / 破損 JSON → warn + backup 生成 / 権限失敗系
- [x] `job-store.memory.test.ts` — add/remove/list 往復
- [x] `schemas.test.ts` — 正常最小 / 完全 / 不正 cron / empty prompt / 不正 timezone
- [x] `clock.test.ts` — FakeClock advance 登録順 fire / clearTimeout 反映 / SystemClock monotonic

### 品質ゲート
- [x] `pnpm -F @proj-airi/cron-runtime typecheck` pass
- [x] `pnpm -F @proj-airi/cron-runtime exec vitest run` all pass (33 / 33), coverage ≥ 80% (91%)
- [x] `eslint` 0 errors
- [x] **非破壊**: `start()` しないと一切タイマは動かない。OC-AG-4 で feature flag と連動

### 実績ログ

#### 2026-04-17 — OC-AG-3 完了
- `packages/cron-runtime/` 新規追加 (`@proj-airi/cron-runtime`): platform-agnostic cron scheduler。
- **公開 API**: `createCronScheduler` / `createMemoryJobStore` / `createJsonJobStore` / `SystemClock` / `createFakeClock` / `cronJobSchema`。
- **単一タイマ再計画**: `setTimeout` は常に 1 本のみ。fire → `reschedule()` で次の最近接を計算して再登録 (polling なし)。
- **In-memory cache authoritative**: fire path は同期 — `store.load()` を callback 内で呼ばない。`FakeClock.advance()` が同期的に fire を観測できる条件。
- **Cache persistence**: 書き込みは fire-and-forget (`store.save().catch`) で同期 callback を阻害しない。
- **CRUD helper**: `addJob` / `removeJob` / `toggleJob` の `store.save` + `reschedule` を `persistAndReschedule()` に抽出。
- **Corrupt JSON 回復**: 破損ファイルを `.corrupt-<ts>` に rename、`[]` を返して続行。
- **FakeClock 再入性**: `advance()` 中に fire した callback が新規タイマを登録した場合、その仮想時刻までのものはその場で fire する。
- **レビュー体制**: 並列 3 エージェントの指摘を反映済み。
- **テスト**: 33 / 33 pass、coverage 91%。

### 申し送り事項
- Electron suspend/resume 時の drift 対策: OC-AG-4 で `powerMonitor.resume` に hook し、新規 API `scheduler.rescheduleAll()` (要追加) で全 job の `nextRunAt` を再計算する。
- 失敗ジョブの retry / backoff は本 Phase では実装せず、OC-AG-8 の拡張候補。
- `onTrigger` は同期 callback。OC-AG-4 で `agent-runtime` に prompt を投げる実装と結線する際は、Queue に enqueue して非同期処理へ流すのが安全。

---

## Phase OC-AG-4: Electron 統合 (injeca + Eventa)

**目的**: Phase 1–3 で作ったコアを Electron main プロセスに束ねる。feature flag OFF 時は完全 no-op にする。

**配置**: `apps/stage-tamagotchi/src/main/services/airi/agent/`

### ファイル構成
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/index.ts` — `createAgentManager` (turn lifecycle, enable/disable, abort)
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/electron-service.ts` — `setupAgentManager` (factory) + `createAgentService` (eventa handler registration)
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/types.ts` — `AgentManager`, `SkillsManager`, `CreateAgentManagerDeps`
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/tool-invoker-mcp.ts` — MCP → ToolInvoker bridge
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/approval-gate.ts` — Pending-promise map for UI approval flow
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/model-driver-stub.ts` — Stub for OC-AG-6
- [x] `apps/stage-tamagotchi/src/main/services/airi/skills/index.ts` — SkillRegistry wiring + `~/.airi/skills/` discovery
- [x] `apps/stage-tamagotchi/src/main/services/airi/cron/index.ts` — CronScheduler wiring + `~/.airi/cron/jobs.json`
- [x] `apps/stage-tamagotchi/src/main/services/airi/cron/trigger-bridge.ts` — job trigger → `agent.startTurn` fire-and-forget
- [x] `apps/stage-tamagotchi/src/shared/agent-runtime.ts` — 共有型 (StartTurnInput, StartTurnResult, AgentRuntimeStatus, ApprovalDecision)
- [x] `apps/stage-tamagotchi/src/shared/eventa.ts` — 14 eventa contracts (11 invoke + 3 broadcast)
- [x] `apps/stage-tamagotchi/src/main/configs/global.ts` — `agentRuntime: { enabled: false }` 追加
- [x] `apps/stage-tamagotchi/src/main/index.ts` — injeca wiring (skills-manager + agent-manager providers, 全 window に agentManager 注入)
- [x] Window RPC files (chat/settings/main) — `createAgentService` 登録

### Feature flag
- [x] `globalAppConfigSchema` に `agentRuntime.enabled: boolean = false` 追加 (Valibot, JSON persistence)
- [x] 全 IPC handler が enabled=false 時に `agent-runtime-disabled` / 空配列を返す
- [x] `main/index.ts` は enabled 値に依存せず常に provide (内部 toggle で再起動不要)

### テスト
- [ ] テストは OC-AG-8 で一括追加予定 (純粋 TS のコアパッケージは 118 テスト済み)

### 品質ゲート
- [x] `pnpm -F @proj-airi/stage-tamagotchi typecheck` pass (node + web)
- [x] `pnpm lint:fix` 0 errors (7 pre-existing warnings)
- [x] コアパッケージテスト 118/118 pass
- [x] **非破壊**: feature flag OFF でアプリ起動 → 既存挙動が完全に維持される

### 実績ログ (2026-04-17)
- 新規ファイル 10 本作成 (`agent/` 6, `skills/` 1, `cron/` 2, `shared/` 1)
- 既存ファイル 9 本修正 (eventa contracts, global config, main index, 3 window index + 3 window RPC)
- Type error 4 件修正 (SystemClock non-constructable, config spread agentRuntime undefined, unused import)
- コードレビュー修正: `errorMessageFrom` 採用、dead code 削除 (status() の fake instanceof check)、banner comment 除去
- 申し送り: approval-gate の `emit` と cron trigger bridge の `onCronTriggered` は no-op stub → OC-AG-5/7 で renderer に結線

---

## Phase OC-AG-5: Renderer Pinia Store + 最小 UI

**目的**: 設定画面で Agent runtime の ON/OFF、スキル一覧、Cron ジョブ管理ができる。devtools ページで agent runs の live log を見る。

**配置** (実装結果 — 共有パッケージ方式):
- Pinia: `packages/stage-ui/src/stores/modules/agent-runtime.ts` (inline eventa contracts)
- 設定: `packages/stage-pages/src/pages/settings/modules/agent-runtime.vue`
- devtools: `apps/stage-tamagotchi/src/renderer/pages/devtools/agent-runs.vue`
- modules list: `packages/stage-ui/src/composables/use-modules-list.ts` (agent-runtime entry 追加)

### ファイル構成
- [x] `packages/stage-ui/src/stores/modules/agent-runtime.ts` — Pinia setup store: `enabled`, `skills`, `cronJobs`, `activeTurns`, `configured`. Inline eventa contracts (7 channels). Actions: refreshStatus, setEnabled, reloadSkills, listCronJobs, addCronJob, removeCronJob, toggleCronJob
- [x] `packages/stage-pages/src/pages/settings/modules/agent-runtime.vue` — enable toggle / skill list (reload button) / cron jobs CRUD form (id, name, cron, prompt, enabled)
- [x] `apps/stage-tamagotchi/src/renderer/pages/devtools/agent-runs.vue` — Runtime status cards (enabled, configured, activeTurns) + Skills summary + Cron Jobs summary
- [x] `packages/stage-ui/src/composables/use-modules-list.ts` — menu entry 追加 (既存 modules セクション、icon: `i-solar:cpu-bolt-bold-duotone`)
- [x] route 設定 — unplugin-vue-router auto-discovery でファイル配置のみで route 登録済

### UI 最小機能
- **設定ページ**: 
  - Enable toggle (feature flag)
  - Skill list (id / name / description / triggers preview) + "Reload from workspace" ボタン
  - Cron jobs CRUD (name / cron expression / prompt / skillId optional / enabled)
- **devtools ページ**:
  - Active runs (turnId / status / elapsed)
  - 選択した run の partial-reply stream + event log
  - Cancel / Approve / Reject ボタン (approval required 時)

### テスト
- [ ] `stores/agent-runtime.test.ts` — Pinia store: initial fetch / event ingestion / add/remove cron (6 ケース)
- [ ] `composables/agent-runtime.test.ts` — composable wrapper (3 ケース)
- [ ] Vue component の visual test は histoire (`packages/stage-ui/stories`) で設定ページ snapshot を登録

### 品質ゲート
- [x] `pnpm -F @proj-airi/stage-ui typecheck` pass
- [x] `pnpm -F @proj-airi/stage-tamagotchi typecheck` pass (node + web)
- [x] `pnpm -F @proj-airi/stage-pages typecheck` pass
- [ ] 設定画面 / devtools ページへ遷移して手動 smoke test (feature flag OFF / ON 両状態)
- [ ] **非破壊**: feature flag OFF 状態の既存チャット / アバター UI に視覚的回帰なし

### 申し送り事項
- UnoCSS shortcut で `agent-run-card` / `skill-badge` 等を `uno.config.ts` に追加することを検討 (OC-AG-8 で整理)
- i18n キーは OC-AG-8 で `packages/i18n` に正式登録、本 Phase は英語ハードコードで OK
- composable (`useAgentRuntime()`) は不要と判断 — store actions で十分。OC-AG-7 の approval UI で必要になれば追加
- devtools ページは現時点で status / skill / cron の read-only ビュー。live run log / cancel は OC-AG-7 で追加

### 実績ログ (2026-04-17)

- **設計変更**: 当初計画の `apps/stage-tamagotchi` 固有ストア → 共有パッケージ方式に変更。Pinia store は `packages/stage-ui/src/stores/modules/agent-runtime.ts` に配置し、stage-web でも再利用可能に
- Pinia store: inline eventa contracts (7 channels) で `packages/stage-ui` が `apps/stage-tamagotchi` の shared eventa に依存しない設計
- 設定ページ: `packages/stage-pages` に配置。Enable toggle + Skills list (reload) + Cron Jobs CRUD (id/name/cron/prompt/enabled)
- devtools ページ: Runtime Status / Skills / Cron Jobs の read-only サマリービュー
- modules list 登録: `use-modules-list.ts` に agent-runtime エントリ追加 (icon: `i-solar:cpu-bolt-bold-duotone`)
- `AgentRuntimeCronJob.name` フィールドの追加対応: store 型にはあったが settings form に欠けていた → form + validation に追加
- 品質: typecheck 3 パッケージ pass (stage-ui, stage-tamagotchi, stage-pages)、lint 0 errors

---

## Phase OC-AG-6: Model Driver bridge (既存 Provider 再利用) ✅

**目的**: `ModelDriver` interface を既存 Airi providers に橋渡しする。OpenClaw の「プロバイダ解決後にハーネスで実行」モデルを Airi に写像。

**採用**: **Option A (renderer 主体)** — main は CronScheduler + SkillRegistry のみ。AgentHarness / ModelDriver / ToolInvoker / ApprovalGate は renderer に寄せる。既存 provider エコシステム (useProvidersStore / useConsciousnessStore) と直接結合し、IPC ラウンドトリップを 0 に抑える。

### 実績構成
- [x] `packages/agent-runtime/src/types.ts` — `ModelDriver` / `DriverEvent` / `ToolInvoker` / `ApprovalGate` interface (OC-AG-1 で先行投入済み)
- [x] `packages/stage-ui/src/libs/providers/agent-driver-bridge.ts` — xsai `streamText` → `ModelDriver` bridge。push-based onEvent を promise-based notification queue で pull-based `AsyncIterable<DriverEvent>` に変換。`.steps.then()` で tool-call 時の finish event を合成
- [x] `packages/stage-ui/src/stores/modules/agent-runtime.ts` — renderer-side harness owner。`dispatchTurn()` が provider 解決 → driver 構築 → harness 実行までを司る。`agentRuntimeCronTriggered` を `ctx.on` で購読し cron 発火時に自動 dispatch
- [x] `apps/stage-tamagotchi/src/main/services/airi/cron/trigger-bridge.ts` — cron 発火 → `CronBroadcastPayload { jobId, turnId, prompt, skillId? }` を全 renderer に emit
- [x] `apps/stage-tamagotchi/src/main/services/airi/agent/{index,types,electron-service}.ts` — main-side は `registerCronBroadcaster(fn)` + `broadcastCronTrigger(payload)` のみを公開。旧 `startTurn / cancelTurn / approvalResolve` 系は削除
- [x] `apps/stage-tamagotchi/src/shared/eventa.ts` — `agentRuntimeStartTurn / CancelTurn / ApprovalResolve / PartialReply / AgentEvent` を削除、`agentRuntimeCronTriggered` を新規追加
- [x] 旧 main-side ファイル削除: `model-driver-stub.ts` / `tool-invoker-mcp.ts` / `approval-gate.ts`

### 最終アーキテクチャ
```
main:
  SkillRegistry (file watcher)
  CronScheduler (fire → trigger-bridge → registerCronBroadcaster 経由で全 renderer に emit)
renderer (chat / settings / main window):
  useAgentRuntimeStore.subscribeCronTriggers()
    → dispatchTurn({ turnId, prompt, jobId })
        → resolveChatProvider() (useConsciousnessStore + useProvidersStore)
        → createXsaiModelDriver() (stage-ui bridge)
        → createAgentHarness({ modelDriver, toolInvoker, approvalGate })
        → harness.runAttempt() — local text/tool events を turnRecords に蓄積
```

### xsai bridge 実装メモ
- `stepCountAtLeast(1)` で 1 ラウンドに制限し、xsai が `execute()` を呼ぶのを受けても harness が先に tool-call を握るようにする
- tool の `execute: () => ''` を必ず登録する (xsai 仕様上 required)。no-op 結果は harness ループで使わない
- tool-call 経路では xsai が `finish` event を出さないので `streamResult.steps` で last step の `finishReason` / `usage` を拾って合成
- abort は `streamText({ abortSignal })` に渡すだけで cooperative cancel される

### テスト
- [x] `packages/stage-ui/src/libs/providers/agent-driver-bridge.test.ts` (4 ケース) — text-delta → finish、tool-call → 合成 finish、step reject → error、systemPrompt が leading message に入る

### 品質ゲート
- [x] `pnpm -F @proj-airi/agent-runtime typecheck` pass
- [x] `pnpm -F @proj-airi/stage-ui typecheck` pass
- [x] `pnpm -F @proj-airi/stage-tamagotchi typecheck` pass (node + web 両方)
- [x] `pnpm exec vitest run packages/stage-ui/src/libs/providers/agent-driver-bridge.test.ts packages/agent-runtime` — 59/59 pass
- [x] lint:fix 適用済（新規・変更ファイルのみ）。pre-existing workspace warnings は無関係

### 実績ログ (2026-04-18)
- Option A 採用により `StartTurnInput` / `StartTurnResult` / `ApprovalDecision` / `PartialReply` / `AgentEvent` 系 IPC を完全撤去 (`apps/stage-tamagotchi/src/shared/agent-runtime.ts` + `eventa.ts`)
- `createAgentService` の manager 引数型を `AgentManagerWithInternals` から public `AgentManager` に緩和 (window RPC はすべて public 型で渡していたため)
- `registerCronBroadcaster` で複数 window への emit を可能にする fan-out パターンを確立。各 window の `createAgentService` が unregister コールバックを返す
- renderer 側 `turnRecords` / `recentTurns` / `runningControllers` を store に追加し、OC-AG-5 devtools page で直接参照できる状態に
- 申し送り (OC-AG-7): approval UI は `createAutoApprovalGate` を `createInteractiveApprovalGate` に差し替えれば組み込める。`ApprovalGate.request()` は harness から呼ばれるので renderer modal との繋ぎは store 内で完結できる

---

## Phase OC-AG-7: 承認フロー UI (approval)

**目的**: OpenClaw の `systemRunPlan` 承認を Airi で再現。ツール呼び出しのうち sensitive (network / filesystem write / shell exec) なものを実行前にユーザー確認する。

**配置**: `apps/stage-tamagotchi/src/renderer/components/agent/` + harness の approval gate

### ファイル構成
- [x] `packages/agent-runtime/src/approval.ts` — `createInteractiveApprovalGate({ emit, onSettled, timeoutMs })` 実装、`ApprovalRequest / ApprovalDecision / InteractiveApprovalGate` 型を追加
- [x] `packages/agent-runtime/src/sensitivity.ts` — 既存の OC-AG-1 実装をそのまま流用（tool prefix / allow-list 判定）
- [x] `apps/stage-tamagotchi/src/renderer/components/agent/approval-modal.vue` — Teleport ベースのモーダル。tool name / sensitivityReason / input JSON を表示、reject 理由入力、Approve / Reject ボタン
- [x] `apps/stage-tamagotchi/src/renderer/composables/agent-approval.ts` — `useAgentApproval()` が `current / count / approve / reject / resolve` を提供
- [x] Pinia store 拡張 (`packages/stage-ui/src/stores/modules/agent-runtime.ts`): `pendingApprovals: PendingApprovalRecord[]` と `resolveApproval(id, decision)`。`createAutoApprovalGate` を削除し、`dispatchTurn` 内で turn 毎に interactive gate を生成。
- [x] `apps/stage-tamagotchi/src/renderer/App.vue` に `<AgentApprovalModal />` を mount（RouterView 隣接）

### テスト
- [x] `packages/agent-runtime/src/approval.test.ts` — 6 ケース: pending emit + approve / reject reason / timeout / signal abort mid-wait / pre-aborted signal / double-resolve 安定性
- [x] `packages/agent-runtime/src/sensitivity.test.ts` — 既存 10 ケース（OC-AG-1）をそのまま流用
- [x] `apps/stage-tamagotchi/src/renderer/composables/agent-approval.test.ts` — 3 ケース: current/count 導出 / approve / reject の reason フォールバック

### 品質ゲート
- [x] `pnpm -F @proj-airi/agent-runtime exec vitest run` → **61/61 pass**（approval 6 ケース追加）
- [x] `pnpm -F @proj-airi/stage-tamagotchi exec vitest run src/renderer/composables` → **3/3 pass**
- [x] `pnpm -F @proj-airi/agent-runtime build` — dist 再生成、stage-ui が新規 export を解決できる
- [x] `pnpm -F @proj-airi/stage-ui typecheck` / `pnpm -F @proj-airi/stage-tamagotchi typecheck` → **全 pass**
- [x] `pnpm exec eslint --fix` を変更ファイル群に実施 → clean
- [ ] 承認必須 tool を呼ぶサンプル cron を手動実行 → モーダルが上がる → approve で流れる / reject で中断する（**手動検証 pending**）

### 実装メモ (2026-04-18)
- **Gate の寿命管理**: interactive gate は `dispatchTurn` の中で turn 毎に new し、`approvalGatesByTurn: Map<turnId, gate>` に保管。`finally` で `approvalGatesByTurn.delete(turnId)` + その turn に紐づく pending を全て掃除。store は **gate を共有しない**（turn 間レース回避）。
- **approval ID ↔ turn のルーティング**: `approvalTurnByRequestId: Map<requestId, turnId>` を保持して `resolveApproval(id, decision)` から正しい gate に到達。pending 配列の `turnId` と一致。
- **signal abort vs timeout**: signal abort は `createAbortError('approval wait')` で reject（既存 `handleToolCall` が signal.aborted / isAbortError を既に処理済）。timeout は `approved: false, reason: 'approval timed out'` で resolve（手動 reject と同じ意味）。
- **emit / onSettled の failure 分離**: emit が throw しても gate の `request` は絶対に壊れないよう try/catch で握りつぶす。renderer 側の listener のバグで agent ループが停止するのを防ぐ。

### 設計判断
- **approval-modal.test.ts の形**: PLAN 初稿では Vue Test Utils で DOM クリックを検証する想定だったが、monorepo に `@vue/test-utils` 依存が一切無く（既存テストは server / scene-canvas など全て非 Vue）、依存追加は scope 肥大。代わりに `composable` の単体テストで `approve` / `reject` が store を正しく叩くことを検証し、DOM レイヤは手動検証に委譲。
- **auto-approval fallback を廃止**: `createAutoApprovalGate` を残すと本番で承認フローがバイパスされるリスクがあるため完全削除。sensitive でない tool は `evaluateSensitivity` が `requiresApproval: false` を返す段階でそのまま通るので、モーダルは本当に必要な時だけ上がる。

### 申し送り事項 (OC-AG-8)
- **Pending 承認の可視化**: 現状 modal しかないが、同時に 2 件以上 pending になった場合 `+N queued` 表示のみ。devtools の agent-runs ページに pending 一覧セクションを足すと運用しやすい（PLAN の OC-AG-8 で検討）。
- **「この skill は自動承認」チェックボックス**: 承認モーダルに `rememberDecision` 切替を入れるなら、`SkillDefinition.allowed` を runtime に反映する flow が必要。OC-AG-8 の拡張案として残す。
- **i18n**: 承認 UI は英語ハードコード。`settings.modules.agent-runtime.approval.*` キーを OC-AG-8 で追加予定（title / reason / approve / reject / inputLabel / rejectReasonPlaceholder / queuedCount）。
- **Cron trigger 経由の承認**: cron が夜間に走って承認待ちで詰まると困るため、timeout 設定を skill 単位で持たせるか、cron job 側に `autoRejectAfterMs` を足すか、OC-AG-8 の UX 検討事項。現状 timeout は未設定（`timeoutMs` undefined = 無期限待機）。

---

## Phase OC-AG-8: テスト + i18n + ドキュメント

**目的**: 全 Phase の統合テスト、i18n 正規登録、README 整備、CLAUDE.md / AGENTS.md への参照追加。

### i18n
- [x] `packages/i18n/src/locales/en/settings.yaml` に `modules.agent-runtime.*` キー
- [x] `packages/i18n/src/locales/ja/settings.yaml` 日本語訳
- [x] `packages/i18n/src/locales/zh-Hans/settings.yaml` 简体中文訳
- [x] 最低キー: `title` / `description` / `enable` / `skills.*` / `cron.*` / `approval.*` を全言語で追加。承認モーダル (`approval-modal.vue`) と `use-modules-list.ts` は `t()` 経由に置き換えた。

### ドキュメント
- [x] `packages/agent-runtime/README.md` — 何をするか / 使い方 / いつ使わないか (AGENTS.md 方針)。OC-AG-7 で追加した `createInteractiveApprovalGate` を反映。
- [x] `packages/skill-registry/README.md` — 同上
- [x] `packages/cron-runtime/README.md` — 同上
- [x] `docs/integrations/agent-runtime.md` — Option A architecture / IPC contract 一覧 / harness lifecycle 表
- [x] `docs/integrations/skill-authoring-guide.md` — frontmatter schema / trigger の書き方 / allow-list の落とし穴 / ローカル検証手順
- [x] `docs/integrations/agent-runtime-security.md` — sensitivity 分類 / allow-list 抽出順 / approval gate 契約 / abort-timeout-maxToolCalls の 3 段 fuse

### 統合テスト
- [x] `apps/stage-tamagotchi/test/integration/agent-runtime.test.ts` — skill discovery + cron scheduler + trigger-bridge の 2 テストを `AIRI_TEST_AGENT_RUNTIME=1` でゲート
- [x] vitest の include パターンに `test/**/*.test.ts` を追加 (`apps/stage-tamagotchi/vitest.config.ts`)
- [ ] Smoke test checklist (手動): 実行は PR レビュー時に別セッションで実施予定
  - [ ] feature flag OFF でチャット / アバター / Claude Code Integration が完全に無変化
  - [ ] enable ON → skill reload → skill が list に出る
  - [ ] cron add → 30s 後トリガ → agent run が devtools に出る
  - [ ] sensitive tool 呼び出し → approval modal 表示 → approve で流れる
  - [ ] abort → tool-invoker.cancel が呼ばれる

### CLAUDE.md / AGENTS.md 更新
- [x] `CLAUDE.md` に OC-Features のエントリ追加（関連 key paths + docs + gated 統合テストを列挙）
- [x] `AGENTS.md` の "Key Path Index" に "Agent runtime channel" サブブロックを追加

### 品質ゲート
- [x] `agent-runtime` / `skill-registry` / `cron-runtime` / `stage-ui` / `stage-tamagotchi` の typecheck pass
- [x] `agent-runtime` (61) / `skill-registry` (30) / `cron-runtime` (33) / 統合テスト (2、gate ON) tests pass
- [x] `stage-ui` は 2 件 pre-existing failure (`execute-tool.test.ts`, `use-vision-inference.test.ts`) のみ。今回の変更と無関係
- [x] `stage-tamagotchi` は 4 件 pre-existing failure (`plugins/index.test.ts`, isolated-vm 未ビルド) のみ。今回の変更と無関係
- [x] 変更ファイルに対する `eslint --fix` 0 errors
- [ ] `pnpm -F @proj-airi/stage-tamagotchi build` は未実施 (renderer/native ビルドは別セッション推奨)
- [x] **非破壊**: 新 i18n キーは追加のみ。`use-modules-list.ts` は agent-runtime エントリのみ `t()` へ差し替え、他モジュール (claude-code / consciousness / ...) は無改変

### 実績ログ (2026-04-18)

- **i18n**: en/ja/zh-Hans の `modules.agent-runtime.*` を追加。既存キーは無改変。approval-modal.vue と use-modules-list.ts が `t()` を使うように変更。typecheck (stage-ui + stage-tamagotchi) 完走。
- **READMEs**: 既存 3 本を現状コード (OC-AG-7 完了状態) に合わせて更新。agent-runtime README に `createInteractiveApprovalGate` を明記し、"OC-AG-4 で実装予定" のような過去形になっていた記述を現在状態に書き換え。
- **Integration docs**: `docs/integrations/` 配下に 3 本を新規追加。ASCII アートを避け、表と code block 中心で構成。architecture / skill-authoring / security の 3 角度をカバー。
- **統合テスト**: `apps/stage-tamagotchi/test/integration/agent-runtime.test.ts` にスカフォールド。skill-registry の discovery→resolve と、cron-runtime の fake clock→scheduler fire→trigger-bridge broadcast を通す。`AIRI_TEST_AGENT_RUNTIME=1` で opt-in、未設定時は `describe.skip` で 0 副作用。
- **型修正**: 最終 typecheck で `packages/agent-runtime/src/approval.ts` の `pending` 型不整合 (`Map<string, PendingEntry>` vs `ReadonlyMap<string, ApprovalRequest>`) が発覚。`pendingView: Map<string, ApprovalRequest>` を並行管理して公開側の型を厳密化。既存 61 テスト全 pass。
- **CLAUDE.md / AGENTS.md**: OC-Features セクションと "Agent runtime channel" サブブロックを追加。

### 申し送り事項
- 本 Phase 完了後、拡張候補:
  - built-in skills pack (`packages/skills-builtin/`)
  - cron job failure retry / backoff
  - Canvas (A2UI) 相当の VRM ステージ連動ビジュアル
  - 承認のホワイトリスト永続化
  - Agent turn 実行ログの SQLite 永続化 (現状は in-memory)
- Smoke checklist (手動) は未実施。リリース候補をカットする前のセッションで `pnpm -F @proj-airi/stage-tamagotchi dev` を起動し、feature flag off → on → skill reload → cron add のウォークスルーを記録する。
- `stage-ui` / `stage-tamagotchi` の pre-existing failure は本 Phase のスコープ外。`execute-tool.test.ts` は文言ドリフト、`use-vision-inference.test.ts` は timeout テストの fake timer 設定、`plugins/index.test.ts` は isolated-vm の native ビルド欠落が根本原因。切り分け・修正は別タスクで扱う。

---

## OC-Features 全体の実装順序と依存

```
OC-AG-1 (Harness core) ─────┐
OC-AG-2 (Skill Registry) ───┤
OC-AG-3 (Cron)              ├─→ OC-AG-4 (Electron 統合)
                             │       ↓
                             │   OC-AG-6 (Model Driver bridge)
                             │       ↓
                             └─→ OC-AG-5 (Renderer UI)
                                     ↓
                                 OC-AG-7 (Approval UI)
                                     ↓
                                 OC-AG-8 (i18n + docs + 統合テスト)
```

- OC-AG-1 / 2 / 3 は並列実装可能 (すべて packages/ 内の pure TS)
- OC-AG-4 で初めて Electron と結線 → feature flag で既存機能から隔離
- OC-AG-6 のアーキテクチャ判断 (Option A/B) は OC-AG-4 着手前に確定する

## 現在機能の非破壊性を担保する 5 つのチェック

1. **feature flag OFF 時**: アプリ起動ログに agent-runtime 関連のエラー/警告ゼロ
2. **既存 Eventa contract は一切変更しない**: `claudeCode*`, `mcp*`, `chat*`, `tts*` は無改変
3. **既存 Pinia store にフィールド追加しない**: 新規 `agent-runtime.ts` store 単独で完結
4. **既存 Provider registry にフィールド追加しない**: `agent-driver-bridge.ts` は registry を読み取るだけ
5. **既存 i18n キー改名/削除しない**: 新 `modules.agent-runtime.*` のみ追加

各 Phase の実績ログに「非破壊チェック: 1–5 すべて pass」を明記することを運用ルールとする。
